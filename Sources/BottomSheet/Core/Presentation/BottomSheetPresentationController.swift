...
public final class BottomSheetPresentationController: UIPresentationController {
    ...
    private func addPullBarIfNeeded(containerView: UIView) {
        ...
        pullBar.configureColors(handleColor: appearance.handleColor, backgroundColor: appearance.backgroundColor)
...
extension BottomSheetPresentationController: UIViewControllerAnimatedTransitioning {
    ...
    public func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        let updatePullBarFrame = {
            guard case .visible(let appearance) = self.configuration.pullBarConfiguration else { return }

            self.pullBar?.frame.origin.y = presentedView.frame.minY - appearance.height + pixelSize
        }
        ...
        if transitionContext.isInteractive {
            UIView.animate(withDuration: transitionDurationValue, delay: 0, options: .curveLinear, animations: animations, completion: completion)
        } else {
            UIView.animate(withDuration: transitionDurationValue,
                           delay: 0,
